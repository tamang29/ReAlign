 ...
 */
export function isReplaceOperation(operation) {
 ...
 */
export function isSignedOperation(operation) {
...
export class PatchVerifier {
    constructor() {
        ...
        this.cleanup
            .pipe(groupBy((path) => path), mergeMap((group) => group.pipe(debounceTime(PatchVerifier.SUPRESSION_WINDOW))), tap((path) => delete this.waitlist[path]))
            .subscribe();
    }
    /**
