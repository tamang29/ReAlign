...
const calculateTokenRadius = (amountOfTokens, outerRadius) => {
    if (amountOfTokens <= 2) {
...
const calculatePositions = (amountOfTokens, outerRadius) => {
    const positions = [];
    if (amountOfTokens === 1) {
...
export const UMLPetriNetPlaceComponent = ({ element, fillColor }) => {
    ...
    return (React.createElement("g", null,
        React.createElement(ThemedCircle, { cx: "50%", cy: "50%", r: radius, strokeColor: element.strokeColor, strokeWidth: 1, fillColor: fillColor || element.fillColor, fillOpacity: 1 }),
        !displayTokenAsNumber &&
            tokenPositions.map((position, index) => (React.createElement(ThemedCircleContrast, { key: index, cx: radius + position.x, cy: radius + position.y, r: tokenRadius, strokeColor: "none", fillColor: element.strokeColor, fillOpacity: 1 }))),
        displayTokenAsNumber && React.createElement(Text, { fill: element.strokeColor }, element.amountOfTokens),
        displayCapacity && (React.createElement("text", { x: "95%", y: 5, pointerEvents: "none", style: element.textColor ? { fill: element.textColor } : {} },
            "C=",
            element.capacity)),
        React.createElement(Text, { fill: element.textColor, y: element.bounds.height + 15 }, element.name)));
};
