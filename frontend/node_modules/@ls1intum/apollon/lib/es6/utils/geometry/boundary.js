 ...
 */
export function computeBoundingBoxForElements(elements) {
    if (!elements.length) {
        return { x: 0, y: 0, width: 0, height: 0 };
    }
...
export function computeDimension(scale, value, isCircle) {
    if (isCircle && scale === 1) {
        return value * scale;
    }
    else {
 ...
 */
export const areBoundsIntersecting = (bounds, intersectingBounds) => {
    const cornerPoints = [
    ...
    return cornerPoints.some((point) => intersectingBoundsStartX <= point.x &&
