...
const SubscriptionPlanAccordion = ({ toggleSubscription, 
    ...
    const currentSubscriptionEnd = subscription ? new Date(subscription.end) : null;
    ...
    return (
        <Accordion.Item eventKey="0" className="accordion-item">
          ...
          <Accordion.Body>
            {organization?.nextSubscription && (
              <div>Your current subscription runs out on {formattedCurSubEnd}. Your account will then be switched to subscription plan {nextSubLevel}.</div>
            )}
            {!organization?.nextSubscription && (
              <div>Your current subscription will be renewed automatically on {formattedCurSubEnd}.</div>
            )}
            {isSubscriptionOpen && (
