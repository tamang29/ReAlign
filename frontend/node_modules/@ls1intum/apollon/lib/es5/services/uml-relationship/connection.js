...
var Connection = /** @class */ (function () {
    ...
    Connection.computePath = function (source, target, options) {
        var sourcePortPosition = (0, uml_element_1.getPortsForElement)(source.element)[source.direction].add(source.element.bounds.x, source.element.bounds.y);
        var targetPortPosition = (0, uml_element_1.getPortsForElement)(target.element)[target.direction].add(target.element.bounds.x, target.element.bounds.y);
        ...
        if (options.isVariable) {
            var straightPath = Connection.tryFindStraightPath(source, target);
        ...
        var ENTITY_MARGIN = 40;
        // Compute an enlarged version of the source and target rectangles,
        // taking into account the entity margin
        var sourceMarginRect = enlargeRect(source.element.bounds, ENTITY_MARGIN);
        var targetMarginRect = enlargeRect(target.element.bounds, ENTITY_MARGIN);
        ...
        var sourceMarginRect1px = enlargeRect(source.element.bounds, ENTITY_MARGIN - 1);
        var targetMarginRect1px = enlargeRect(target.element.bounds, ENTITY_MARGIN - 1);
        // Calculate the exact position of the start and end points on their respective margin rectangle
        var startPointOnMarginBox = sourcePortPosition.clone();
        ...
        var endPointOnMarginBox = targetPortPosition.clone();
        ...
        var sourceCornerClosestToEndPoint = findClosestPoint(getCorners(sourceMarginRect), endPointOnMarginBox);
        // Determine the target corner that's closest to the previously determined source corner
        var targetCornerClosestToClosestSourceCorner = findClosestPoint(getCorners(targetMarginRect), sourceCornerClosestToEndPoint);
        // Determine the corner queue for the source entity
        var sourceCornerQueue = determineCornerQueue(sourceMarginRect, source.direction, startPointOnMarginBox, sourceCornerClosestToEndPoint);
        // Determine the corner queue for the target entity
        var targetCornerQueue = determineCornerQueue(targetMarginRect, target.direction, endPointOnMarginBox, targetCornerClosestToClosestSourceCorner);
        // The relationship path can be partitioned into two segments:
        // a prefix from the start point and a suffix to the end point
        var pathFromStart = [sourcePortPosition, startPointOnMarginBox];
        var pathFromEnd = [targetPortPosition, endPointOnMarginBox];
        // We build the relationship path up both from the start and the end
        var currentStartPoint = tslib_1.__assign({}, startPointOnMarginBox);
        var currentEndPoint = tslib_1.__assign({}, endPointOnMarginBox);
        while (true) {
            var startAndEndPointCanBeConnected = !lineSegmentIntersectsRect(currentStartPoint, currentEndPoint, sourceMarginRect1px) &&
            ...
            if (startAndEndPointCanBeConnected) {
                var currentStartAxis = getAxisForPathSegment(pathFromStart.slice(-2));
                var currentEndAxis = getAxisForPathSegment(pathFromEnd.slice(-2));
                if (currentStartAxis === 'HORIZONTAL' && currentEndAxis === 'HORIZONTAL') {
                    var middleX = (currentStartPoint.x + currentEndPoint.x) / 2;
                    ...
                    var middleY = (currentStartPoint.y + currentEndPoint.y) / 2;
            ...
            var nextSourceCorner = sourceCornerQueue.shift();
            if (nextSourceCorner !== undefined) {
                ...
                var nextTargetCorner = targetCornerQueue.shift();
        ...
        var pathToEnd = pathFromEnd.reverse();
        var path = tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(pathFromStart), false), tslib_1.__read(pathToEnd), false);
    ...
    Connection.tryFindStraightPath = function (source, target) {
        var OVERLAP_THRESHOLD = 40;
        var sourceHandleEdge = determineHandleEdge(source.direction);
        var targetHandleEdge = determineHandleEdge(target.direction);
        /*
            #######              #######              #######
            # ~~~ # -----------> # ~~~ # -----------> # ~~~ #
            # ~~~ #      |       #######     |----->  # ~~~ #
            ...
            #######                                   #######
        */
        if (sourceHandleEdge === uml_element_port_1.Direction.Right &&
            targetHandleEdge === uml_element_port_1.Direction.Left &&
            target.element.bounds.x >= source.element.bounds.x + source.element.bounds.width) {
            var overlapY = computeOverlap([source.element.bounds.y, source.element.bounds.y + Math.max(OVERLAP_THRESHOLD, source.element.bounds.height)], [target.element.bounds.y, target.element.bounds.y + Math.max(OVERLAP_THRESHOLD, target.element.bounds.height)]);
            if (overlapY !== null && overlapY[1] - overlapY[0] >= OVERLAP_THRESHOLD) {
                var middleY = (overlapY[0] + overlapY[1]) / 2;
                var start = { x: source.element.bounds.x + source.element.bounds.width, y: middleY };
                var end = { x: target.element.bounds.x, y: middleY };
                return [start, end];
            }
        }
        /*
            ...
            #######                                   #######
        */
        if (sourceHandleEdge === uml_element_port_1.Direction.Left &&
            targetHandleEdge === uml_element_port_1.Direction.Right &&
            source.element.bounds.x >= target.element.bounds.x + target.element.bounds.width) {
            var overlapY = computeOverlap([source.element.bounds.y, source.element.bounds.y + Math.max(OVERLAP_THRESHOLD, source.element.bounds.height)], [target.element.bounds.y, target.element.bounds.y + Math.max(OVERLAP_THRESHOLD, target.element.bounds.height)]);
            if (overlapY !== null && overlapY[1] - overlapY[0] >= OVERLAP_THRESHOLD) {
                var middleY = (overlapY[0] + overlapY[1]) / 2;
                var start = { x: source.element.bounds.x, y: middleY };
                var end = { x: target.element.bounds.x + target.element.bounds.width, y: middleY };
                return [start, end];
            }
        }
        /*
            ...
            ##################
        */
        if (sourceHandleEdge === uml_element_port_1.Direction.Down &&
            targetHandleEdge === uml_element_port_1.Direction.Up &&
            target.element.bounds.y >= source.element.bounds.y + source.element.bounds.height) {
            var overlapX = computeOverlap([source.element.bounds.x, source.element.bounds.x + source.element.bounds.width], [target.element.bounds.x, target.element.bounds.x + target.element.bounds.width]);
            if (overlapX !== null && overlapX[1] - overlapX[0] >= OVERLAP_THRESHOLD) {
                var middleX = (overlapX[0] + overlapX[1]) / 2;
                var start = { x: middleX, y: source.element.bounds.y + source.element.bounds.height };
                var end = { x: middleX, y: target.element.bounds.y };
                return [start, end];
            }
        }
        /*
            ...
            ##################
        */
        if (sourceHandleEdge === uml_element_port_1.Direction.Up &&
            targetHandleEdge === uml_element_port_1.Direction.Down &&
            source.element.bounds.y >= target.element.bounds.y + target.element.bounds.height) {
            var overlapX = computeOverlap([source.element.bounds.x, source.element.bounds.x + source.element.bounds.width], [target.element.bounds.x, target.element.bounds.x + target.element.bounds.width]);
            if (overlapX !== null && overlapX[1] - overlapX[0] >= OVERLAP_THRESHOLD) {
                var middleX = (overlapX[0] + overlapX[1]) / 2;
                var start = { x: middleX, y: source.element.bounds.y };
                var end = { x: middleX, y: target.element.bounds.y + target.element.bounds.height };
                return [start, end];
            }
        }
        return null;
...
function findClosestPoint(candidates, target) {
    ...
    try {
        for (var candidates_1 = tslib_1.__values(candidates), candidates_1_1 = candidates_1.next(); !candidates_1_1.done; candidates_1_1 = candidates_1.next()) {
            ...
            var distance = distanceBetweenPoints(target, candidate);
...
function removeConsecutiveIdenticalPoints(path) {
    ...
    try {
        for (var path_1 = tslib_1.__values(path), path_1_1 = path_1.next(); !path_1_1.done; path_1_1 = path_1.next()) {
            ...
            var previousPoint = newPath[newPath.length - 1];
...
function removeTransitNodes(path) {
    for (var i = 0; i < path.length - 2; i++) {
        ...
        if (isHorizontalLineSegment(p, q, r) || isVerticalLineSegment(p, q, r)) {
            var pointsBeforeQ = path.slice(0, i + 1);
            var pointsAfterQ = path.slice(i + 2);
            var pathWithoutQ = tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(pointsBeforeQ), false), tslib_1.__read(pointsAfterQ), false);
            return removeTransitNodes(pathWithoutQ);
        }
    }
    return path;
}
function isHorizontalLineSegment(p, q, r) {
    return (areAlmostEqual(p.y, q.y) && areAlmostEqual(q.y, r.y) && ((p.x >= q.x && q.x >= r.x) || (p.x <= q.x && q.x <= r.x)));
}
function isVerticalLineSegment(p, q, r) {
...
function mergeConsecutiveSameAxisDeltas(path) {
    var e_3, _a;
    var deltas = computePathDeltas(path);
    if (deltas.length <= 1) {
        return path;
    }
    var newDeltas = [];
    try {
        for (var deltas_1 = tslib_1.__values(deltas), deltas_1_1 = deltas_1.next(); !deltas_1_1.done; deltas_1_1 = deltas_1.next()) {
            ...
            var previousDelta = newDeltas[newDeltas.length - 1];
            if (!previousDelta) {
...
function createPathFromDeltas(start, deltas) {
    ...
    var current = start;
    try {
        for (var deltas_2 = tslib_1.__values(deltas), deltas_2_1 = deltas_2.next(); !deltas_2_1.done; deltas_2_1 = deltas_2.next()) {
            var _b = deltas_2_1.value, dx = _b.dx, dy = _b.dy;
            var x = current.x + dx;
            var y = current.y + dy;
            current = { x: x, y: y };
            points.push(current);
        ...
        try {
            if (deltas_2_1 && !deltas_2_1.done && (_a = deltas_2.return)) _a.call(deltas_2);
        }
...
function flattenWaves(path) {
    ...
    var deltas = computePathDeltas(path);
    var simplifiedDeltas = simplifyDeltas(deltas);
    var start = path[0];
    var simplifiedPath = createPathFromDeltas(start, simplifiedDeltas);
...
function simplifyDeltas(deltas) {
    for (var i = 0; i < deltas.length - 3; i++) {
        ...
        if (d1.dy === 0 &&
            ...
            return simplifyDeltas(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(deltas.slice(0, i)), false), [
                { dx: d1.dx + d3.dx, dy: 0 },
                { dx: 0, dy: d2.dy }
            ], false), tslib_1.__read(deltas.slice(i + 3)), false));
        }
        if (d1.dx === 0 &&
            d2.dy === 0 &&
            d3.dx === 0 &&
...
function determineCornerQueue(rect, edge, pointOnOuterEdge, destinationCorner) {
    var clockwiseCornerQueue;
    var counterClockwiseCornerQueue;
    // Get all corners of the rectangle
    var tl = getTopLeftCorner(rect);
    var tr = getTopRightCorner(rect);
    var bl = getBottomLeftCorner(rect);
    var br = getBottomRightCorner(rect);
    ...
    var clockwisePathLength = computePathLength(tslib_1.__spreadArray([pointOnOuterEdge], tslib_1.__read(clockwiseCornerQueue), false));
    var counterClockwisePathLength = computePathLength(tslib_1.__spreadArray([pointOnOuterEdge], tslib_1.__read(counterClockwiseCornerQueue), false));
...
function lineSegmentIntersectsRect(p, q, rect) {
    ...
    return (lineSegmentsIntersect(p, q, topLeftCorner, topRightCorner) ||
 ...
 */
function lineSegmentLiesWithinRect(p, q, rect) {
...
function lineSegmentsIntersect(p1, q1, p2, q2) {
    ...
    if (o1 === 0 /* Orientation.Collinear */ && liesOnSegment(p1, p2, q1)) {
        return true;
    }
    // p1, q1 and p2 are collinear and q2 lies on segment p1q1
    if (o2 === 0 /* Orientation.Collinear */ && liesOnSegment(p1, q2, q1)) {
        return true;
    }
    // p2, q2 and p1 are collinear and p1 lies on segment p2q2
    if (o3 === 0 /* Orientation.Collinear */ && liesOnSegment(p2, p1, q2)) {
        return true;
    }
    // p2, q2 and q1 are collinear and q1 lies on segment p2q2
    if (o4 === 0 /* Orientation.Collinear */ && liesOnSegment(p2, q1, q2)) {
        return true;
    }
...
function getOrientation(p, q, r) {
    ...
    return val > 0 ? 1 /* Orientation.Clockwise */ : 2 /* Orientation.CounterClockwise */;
}
/**
...
function computeOverlap(range1, range2) {
    var _a = tslib_1.__read(range1, 2), from1 = _a[0], to1 = _a[1];
    var _b = tslib_1.__read(range2, 2), from2 = _b[0], to2 = _b[1];
    var largerFrom = Math.max(from1, from2);
    var smallerTo = Math.min(to1, to2);
    return largerFrom <= smallerTo ? [largerFrom, smallerTo] : null;
}
function getAxisForPathSegment(pathSegment) {
    // Determine dx and dy
    var _a = tslib_1.__read(pathSegment, 2), p = _a[0], q = _a[1];
    var dx = q.x - p.x;
    var dy = q.y - p.y;
...
function determineHandleEdge(handle) {
    switch (handle) {
        ...
        case uml_element_port_1.Direction.Upleft:
        case uml_element_port_1.Direction.Downleft:
            return uml_element_port_1.Direction.Left;
        case uml_element_port_1.Direction.Right:
        case uml_element_port_1.Direction.Upright:
        case uml_element_port_1.Direction.Downright:
            return uml_element_port_1.Direction.Right;
        case uml_element_port_1.Direction.Down:
        case uml_element_port_1.Direction.Bottomleft:
        case uml_element_port_1.Direction.Bottomright:
            return uml_element_port_1.Direction.Down;
        case uml_element_port_1.Direction.Up:
        case uml_element_port_1.Direction.Topleft:
        case uml_element_port_1.Direction.Topright:
            return uml_element_port_1.Direction.Up;
        default:
            break;
    }
}
