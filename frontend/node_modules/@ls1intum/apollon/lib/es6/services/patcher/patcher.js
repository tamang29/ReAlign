...
import { buffer, debounceTime, filter, map, merge, Subject, throttleTime } from 'rxjs';
...
export class Patcher {
    ...
    constructor(options = _DefaultOptions) {
        ...
        //
        this.observable = router.pipe(buffer(router.pipe(debounceTime(0))), map((patches) => patches.flat()), filter((patches) => patches.length > 0));
    }
    /**
